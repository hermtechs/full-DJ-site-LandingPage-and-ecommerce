...
function addItemToCart(event){
 ...
 var id = productContainer.dataset.itemId;

 addProductToCart(productName, prodImageUrl, prodPrice, id)
// fndTotal();
openAndCloseCart();
// checkItemAlreadyAdded(productName)
}

function addProductToCart(productName, prodImageUrl, prodPrice, id){
    ...
    ***TO DO***/
    ...
    cartRowContents = `<article class="cart-item" data-item-id="${id}">
...
var stripeHandler = StripeCheckout.configure(
    {
        ...
        token : function(token){
            ...
            cartRows.forEach(row=>{
                const quantity = row.querySelector('.item-quantity').innerText;
            ...
            /*fetching our user's token id that we get from the response stripeHandler sends back after user clicks checkout,
            together with the details of the items user purchased to our server */
            ...
            fetch('/purchase', { 
                ...
                alert(data.message)
                cartItemsContainer.innerHTML = ""; //emptying the cart
                updateTotal();
            }).catch((error)=>{
                console.error(error)
            })
...
function checkOutClicked(){
    ...
    const price = parseFloat(totalPriceElement.innerText)*100; /*multiplying by 100 because stripe expects the amout to be in cents*/
    // console.log(amount);
    stripeHandler.open({
        amount:price
    })
